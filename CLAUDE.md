...
- **Ortak gönderim/loglama yardımcı fonksiyonu:** `src/lib/email/send-email.ts` → `sendAndLogEmail()`, Resend SDK ile gönderir (`src/lib/email/resend.ts`, `server-only`) ve sonucu (başarılı/başarısız, `servis_mesaj_id`, `hata_mesaji`) **her zaman** `email_logs`'a yazar — gönderim başarısız olsa bile satır eklenir, sessizce yutulan hata olmaz (kural 9). İki ayrı akış bu fonksiyonu paylaşır:
  ...
  2. **MYK belgelendirme fırsatı dahili bildirimi**: `markNonCompliantAction` (`src/app/(app)/denetimler/[inspectionId]/actions.ts`) içinde, `is_certification_opportunity=true` olan bir maddeden uygunsuzluk doğduğunda **otomatik olarak** (kullanıcı buton tıklamaz) tetiklenir; `notification_settings.myk_firsat_bildirim_alicilari`'na gider. Bu liste boşsa veya `notification_settings` satırı hiç yoksa **sessizce atlanır** (bu bir hata değil, henüz yapılandırılmamış bir özelliktir — atlanan durum için `email_logs`'a satır yazılmaz, çünkü gönderim denemesi hiç yapılmamıştır). `email_logs.finding_id` doldurulur, `inspection_id`/`report_id` null kalır — bu bildirim doğrudan bulguya bağlıdır, rapora değil (bkz. "Beklenen özellik" bölümü).
...
## Beklenen özellik: MYK belgelendirme fırsatı bildirimi

Kullanıcı isteği: bir denetimde "personelin MYK belgesi eksik" türünde bir uygunsuzluk yazıldığında, MESMER'in kendisine (işletmeye değil) dahili bir bildirim gitmeli — amaç, MESMER'in o işletmeye MYK sınavı hizmeti teklif edebilmesi. Tasarım kararı (Teslim 2/9'da uygulanacak, kullanıcıya bildirildi, itiraz gelmedi):

- `checklist_items.is_certification_opportunity boolean default false` — bir kontrol maddesinin bu türden olduğunu şablon yazarı işaretler.
- Böyle bir maddeden "Uygun Değil" ile uygunsuzluk oluştuğunda, kuruluşun `notification_settings` içindeki dahili alıcı listesine (işletme yetkilisinden bağımsız) anlık e-posta gider.
- `email_logs.finding_id` (nullable) eklenir — bu bildirim rapora/denetime değil doğrudan bulguya bağlanır.
